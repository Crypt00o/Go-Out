...
dynamic_path='function goout(){ current_path=$(pwd); GoOut_Bin_Path=$(sudo locate Go-Out/goout); if [ -f $GoOut_Bin_Path ] && [ ! -z $GoOut_Bin_Path ] ;then GoOut_Path=$(echo $GoOut_Bin_Path | sed "s/\/goout//g"); cd $GoOut_Path; sudo ./goout $* ;cd $current_path; else printf "\n\n[+] Updateing Database of Names , To Locate Goout  \n\n"; sudo updatedb ; GoOut_Bin_Path=$(sudo locate Go-Out/goout); if [ -f $GoOut_Bin_Path ] && [ ! -z $GoOut_Bin_Path ]; then GoOut_Path=$(echo $GoOut_Bin_Path | sed "s/\/goout//g") ;cd $GoOut_Path ; sudo ./goout $* ;cd $current_path ;else echo "[-] Deleted or Corrupted , You need to Download it again to use it " ;printf "\n\nDo You Want to Install it Again ? [y/n] Default=N : "; read answer ;if [[ $answer == "y" ]] || [[ $answer == "Y" ]] || [[ $answer == "yes" ]] || [[ $answer == "Yes" ]] ; then bash -c "$(wget -O- https://raw.githubusercontent.com/Crypt00o/Go-Out/main/install.sh)" 2>/dev/null; else echo "[-] Installation Cancelled"; fi ;fi; fi }'
sudo_function='sudo-function() {sudo bash -c "$(declare -f "$1");$(printf '%q' "$@")"}' 
goout_alias="alias goout='sudo-function goout $*'" 
if [ -f '/bin/bash' ]
then
